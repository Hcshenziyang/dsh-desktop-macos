...
- [ ] 验证主题关闭/恢复默认可以移除全部覆盖；检查浅色、深色、对话、设置、弹窗和代码块
      在最低面板不透明度下仍具备可读性。
- [ ] 验证 `dsh-desktop-theme://` 只读取受管自选壁纸目录和 App Bundle 的内置主题目录，拒绝
      其他 host、目录穿越和跨主题引用；不要提交无明确再分发许可的壁纸。
- [ ] 核对每个 `Resources/Themes/<theme-id>/` 都只含本主题清单、素材和来源说明，清单 ID 与
      目录一致，且没有跨主题资源引用。
